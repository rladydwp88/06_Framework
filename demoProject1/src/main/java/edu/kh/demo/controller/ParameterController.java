...
@Slf4j // log를 이용한 메시지를 콘솔창에 출력할 때 사용(Lombok 제공)
public class ParameterController {
	...
	public String paramTest2(@RequestParam("title") String title,
							...
							// 형변환이 필요한 경우에는 파라미터 value값 필수 작성되어 있어야함.
							@RequestParam(value="publisher", required=false, defaultValue="출판사") String publisher) {
	...
	public String paramTest3(@RequestParam("color") String[] colorArr,
		...
		// @RequestParam Map<String, Object> paramMap
		// -> 제출 된 모든 파라미터가 Map에 저장된다
		// -> 같은 name 속성을 가진 파라미터는 String[], List로 저장 X
		//	  첫번째로 제출된 value값이 저장됨
		log.debug("paramMap : " + paramMap);
		//{color=Red, fruit=Apple, productName=아름다운조명, expirationDate=2025-04-16}
		
		return "redirect:/param/main";
	}
	
	// 4. @ModelAttrubute를 이용한 파라미터 얻어오기
	// @ModelAttrubute
	...
	public String paramTest4(/*@ModelAttribute*/ MemberDTO inputMember) {
		...
		// == "redirect:/param/main"
		// 상대경로 작성법 (현재 위치가 중요! -> 현재 경로의 가장 마지막 부분을 redirect:이 주소로 변경하기 때문에)
	}
}
