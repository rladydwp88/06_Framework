...
if (updateInfo != null) {
    ...
    updateInfo.addEventListener("submit", async e => {
        ...
        /*
        submit 이벤트 핸들러 함수 내에서 fetch를 이용한 비동기 함수를 실행 중일때
...
if(profileForm != null) {
    ...
    imageInput.addEventListener("change", () => {
        ...
        if(file) { // 파일이 선택이 된 경우
            if(file.size <= MAX_SIZE) { // 현재 선택한 파일의 크기가 허용범위 이내인 경우
                ...
                previousFile = file; // 현재 선택된 파일 객체를 이전 파일로 저장(다음에 바뛸 일에 대비)
            ...
            if(previousFile) { // 이전 파일이 존재한다면
                ...
                // --> input 태그의 files 속성 FileList만 저장 가능한 형태이기 때문에
    ...
    deleteImage.addEventListener("click", () => {
        ...
        if(profileImg.src !== defaultImageUrl) {
            ...
            previousImage = defaultImageUrl; // 이전 이미지 기록하는 변수에 기본 이미지로 변경
