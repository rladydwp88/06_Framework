...
public class BoardServiceImpl implements BoardService {
	...
	public int boardLike(Map<String, Integer> map) {
		...
		// 1. 좋아요가 체크 된 상태인 경우(likeCheck == 1)
		// -> BOARD_LIKE 테이블에 DELETE 수행
		if(map.get("likeCheck") == 1) {
			result = mapper.deleteBoardLike(map);
			
		} else {
		// 2. 좋아요가 해제 된 상태인 경우(likeCheck == 0)
	...
	public int updateReadCount(int boardNo) {
		...
		// 2. 현재 조회 수 조회
		if(result > 0) {
			return mapper.selectBoardCount(boardNo);
		}
	...
	// 전체 검색 서비스
	@Override
	public List<Board> selectSearchList(int boardCode, String key, String query) {
		
		return mapper.selectSearchList(boardCode, key, query);
