...
-> 어떤 결과 올지는 모르지만 반드시 결과를 보내주겠다는 약속.
-> 비동기 작업이 맞이할 완료 또는 실패와 그 결고값을 나타냄.
...
addBtn.addEventListener("click", () => {
  ...
  // {"name" : "홍길동", "age" : "20", "skill" : ["javascript"]}
  
  // todoTitle과 todoCountent를 저장한 JS 객체
  const param = {
    // key : value
  ...
  .then(resp => resp.text()) // 반환 된 값을 text로 변환
  .then(result => {
    // 처번째 then에서 반환된 값을 result 에 저장
    console.log(result);
