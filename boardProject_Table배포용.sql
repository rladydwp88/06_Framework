...
VALUES(SEQ_MEMBER_NO.NEXTVAL, 
			 'admin02@kh.or.kr',
			 '$2a$10$MEUPnIOKqQjwSU.nW0KEH.O8Wz92LJnfh4NqB34QKqnglotrDS6Ui',
			 '김관리',
			 '01012341234',
			 NULL,
			 NULL,
...
-- 암호화 된 비밀번호(평문 : pass01!) 업데이트
UPDATE "MEMBER" SET
MEMBER_PW = '$2a$10$c4st2BHSEHQcVhfqYB89p.sI3vzPHgTThKl5JcQXB97V3mMEN1FXi'
WHERE MEMBER_NO = 1;
...
MEMBER_NICKNAME, TO_CHAR(FILE_UPLOAD_DATE, 'YYY-MM-DD') FILE_UPLOAD_DATE
FROM "UPLOAD_FILE"
 ...
 * 
 * 특정 사용자가 참여한 채팅방 목록, 각 채팅방의 최근 메시지, 관련된 정보 조회 목
 * */
SELECT CHATTING_ROOM_NO
	...
	,NVL2((SELECT OPEN_MEMBER FROM CHATTING_ROOM R2
		...
		R.PARTICIPANT, -- NULL 이 아닌 경우 (로그인한 사람 오픈멤버일때는 참가자 타켓번호)
		R.OPEN_MEMBER  -- NULL 인 경우    (내가 오픈 멤버가 아닐때는 OPEN_MEMBER가 타켓번호)
		) TARGET_NO	
	,NVL2((SELECT OPEN_MEMBER FROM CHATTING_ROOM R2
		WHERE R2.CHATTING_ROOM_NO = R.CHATTING_ROOM_NO
		AND R2.OPEN_MEMBER = 1),
